...
class GlazovDrone(Drone):
    ...
    def get_target(self):
        ...
        # enemies = [(mothership, self.distance_to(mothership)) for mothership in self.scene.motherships if
        #                 mothership.team != self.team and mothership.is_alive and mothership.team_number == 4]
    ...
    def get_place_for_attack(self, soldier, target):
        ...
        angles = [0, 60, -60, 30, -30, 45, -45, 15, -15, 75, -75]
    ...
    def _get_my_asteroid(self, dist):
        not_empty_asteroids = [asteroid for asteroid in self.scene.asteroids]
    ...
    def move_at(self, target, speed=None):
        if not self.is_alive:
            return
        super(Drone, self).move_at(target, speed=100)
    ...
    def get_target_and_move(self):
        ...
        # if self.fighter_target and not self.destination:
        #     self.destination = self.get_place_for_attack(self, self.fighter_target)
        #     self.move_at(self.destination)
        #
        # elif self.fighter_target and self.destination:
        #     self.move_at(self.destination)
        #
        # elif not self.fighter_target and self.destination:

        self.target = self.get_target()
        # if isinstance(self.fighter_target, GameObject):
    ...
    def on_unload_complete(self):
        if self.job == 'worker':
            self.move_at(self._get_my_asteroid(dist='distance_near'))
    ...
    def __min_distance(self, not_empty_asteroids):
        ...
        target = None
        for asteroid in not_empty_asteroids:
            if asteroid.payload > 0 and self.distance_to(asteroid) < min_distance:
